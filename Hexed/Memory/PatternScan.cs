...
namespace Hexed.Memory
{
    internal class PatternScan
    {
        ...
        public IntPtr Find(byte[] pattern, int patternOffset, int addressOffset, ScanMethod method)
        {
            ...
            if (address < 1) return IntPtr.Zero;

            switch (method)
            {
                case ScanMethod.None:
                    return address + patternOffset + addressOffset;
        ...
        private byte[] MaskToPattern(string mask)
        {
            ...
            {
                if (numbers[i].StartsWith("?")) pattern[i] = 0x00;
                else pattern[i] = byte.Parse(numbers[i], NumberStyles.HexNumber);
            }
